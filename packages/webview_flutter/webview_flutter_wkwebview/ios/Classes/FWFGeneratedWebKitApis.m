...
@interface FWFPWebHistoryItem ()
+ (FWFPWebHistoryItem *)fromList:(NSArray *)list;
+ (nullable FWFPWebHistoryItem *)nullableFromList:(NSArray *)list;
- (NSArray *)toList;
@end

@interface FWFPWebHistory ()
+ (FWFPWebHistory *)fromList:(NSArray *)list;
+ (nullable FWFPWebHistory *)nullableFromList:(NSArray *)list;
- (NSArray *)toList;
@end
...
@implementation FWFPWebHistoryItem
+ (instancetype)makeWithOriginalUrl:(nullable NSString *)originalUrl
    title:(nullable NSString *)title
    url:(nullable NSString *)url
    index:(nullable NSNumber *)index
    offset:(nullable NSNumber *)offset {
  FWFPWebHistoryItem* pigeonResult = [[FWFPWebHistoryItem alloc] init];
  pigeonResult.originalUrl = originalUrl;
  pigeonResult.title = title;
  pigeonResult.url = url;
  pigeonResult.index = index;
  pigeonResult.offset = offset;
  return pigeonResult;
}
+ (FWFPWebHistoryItem *)fromList:(NSArray *)list {
  FWFPWebHistoryItem *pigeonResult = [[FWFPWebHistoryItem alloc] init];
  ...
  pigeonResult.offset = GetNullableObjectAtIndex(list, 4);
  return pigeonResult;
}
+ (nullable FWFPWebHistoryItem *)nullableFromList:(NSArray *)list {
  return (list) ? [FWFPWebHistoryItem fromList:list] : nil;
}
- (NSArray *)toList {
  return @[
    ...
    self.offset ?: [NSNull null],
  ];
}
@end

@implementation FWFPWebHistory
+ (instancetype)makeWithHistory:(nullable NSArray<FWFPWebHistoryItem *> *)history
    currentIndex:(nullable NSNumber *)currentIndex {
  FWFPWebHistory* pigeonResult = [[FWFPWebHistory alloc] init];
  pigeonResult.history = history;
  pigeonResult.currentIndex = currentIndex;
  return pigeonResult;
}
+ (FWFPWebHistory *)fromList:(NSArray *)list {
  FWFPWebHistory *pigeonResult = [[FWFPWebHistory alloc] init];
  pigeonResult.history = GetNullableObjectAtIndex(list, 0);
  pigeonResult.currentIndex = GetNullableObjectAtIndex(list, 1);
  return pigeonResult;
}
+ (nullable FWFPWebHistory *)nullableFromList:(NSArray *)list {
  return (list) ? [FWFPWebHistory fromList:list] : nil;
}
- (NSArray *)toList {
  return @[
    self.history ?: [NSNull null],
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    ...
    case 135: 
      return [FWFObjectOrIdentifier fromList:[self readValue]];
    case 136: 
      return [FWFPWebHistory fromList:[self readValue]];
    case 137: 
      return [FWFPWebHistoryItem fromList:[self readValue]];
    case 138: 
      return [FWFWKAudiovisualMediaTypeEnumData fromList:[self readValue]];
    case 139: 
      return [FWFWKFrameInfoData fromList:[self readValue]];
...
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[FWFAuthenticationChallengeResponse class]]) {
    ...
    [self writeByte:135];
    [self writeValue:[value toList]];
  } else if ([value isKindOfClass:[FWFPWebHistory class]]) {
    [self writeByte:136];
    [self writeValue:[value toList]];
  } else if ([value isKindOfClass:[FWFPWebHistoryItem class]]) {
    [self writeByte:137];
    [self writeValue:[value toList]];
...
void SetUpFWFWKWebViewHostApi(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FWFWKWebViewHostApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.webview_flutter_wkwebview.WKWebViewHostApi.getCopyBackForwardList"
        binaryMessenger:binaryMessenger
        codec:FWFWKWebViewHostApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(getCopyBackForwardListForWebViewWithIdentifier:error:)], @"FWFWKWebViewHostApi api (%@) doesn't respond to @selector(getCopyBackForwardListForWebViewWithIdentifier:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSInteger arg_identifier = [GetNullableObjectAtIndex(args, 0) integerValue];
        FlutterError *error;
        FWFPWebHistory *output = [api getCopyBackForwardListForWebViewWithIdentifier:arg_identifier error:&error];
        callback(wrapResult(output, error));
      }];
    } else {
      [channel setMessageHandler:nil];
